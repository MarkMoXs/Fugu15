...
uint64_t kernel_mount(const char *fstype, uint64_t pvp, uint64_t vp, const char *mountPath, uint64_t data,
                      size_t datalen, int syscall_flags, uint32_t kern_flags) {
  ...
  uint64_t ret = kcall(kernel_mount_kaddr, 9,
                       (uint64_t[]){kern_fstype, pvp, vp, kern_mountPath, data, datalen, syscall_flags,
                                    kern_flags, kerncontext_kaddr});
...
uint64_t bindMount(const char *source, const char *target) {
  ...
  uint64_t mount_ret =
      kernel_mount("bindfs", parent_vnode, vnode, targetPath.fileSystemRepresentation,
                   (uint64_t)targetPath.fileSystemRepresentation, 8, MNT_RDONLY, KERNEL_MOUNT_NOAUTH);
...
void generateSystemWideSandboxExtensions(NSString *targetPath) {
  ...
  extension =
      sandbox_extension_issue_mach("com.apple.app-sandbox.mach", "com.opa334.jailbreakd.systemwide", 0);
  if (extension) [extensions addObject:[NSString stringWithUTF8String:extension]];
  extension = sandbox_extension_issue_mach("com.apple.security.exception.mach-lookup.global-name",
                                           "com.opa334.jailbreakd.systemwide", 0);
  if (extension) [extensions addObject:[NSString stringWithUTF8String:extension]];
...
void increaseJetsamLimit(pid_t pid) {
  ...
  JBLogDebug("JETSAM %d previous limit (%u/%u)", pid, mmprops.v1.memlimit_active,
             mmprops.v1.memlimit_inactive);

  // mmprops.v1.memlimit_active = mmprops.v1.memlimit_active * 10;
  // mmprops.v1.memlimit_inactive = mmprops.v1.memlimit_inactive * 10;

  // for whatever fucking reason it expects the v1 struct when setting but gives you the v2 struct when
  // getting don't ask me why lol
...
int64_t initEnvironment(NSDictionary *settings) {
  ...
  copySuc = [[NSFileManager defaultManager] copyItemAtPath:@"/var/jb/basebin/systemhook.dylib"
                                                    toPath:@"/var/jb/basebin/.fakelib/systemhook.dylib"
                                                     error:nil];
  if (!copySuc) {
    return 7;
  }
...
void jailbreakd_received_message(mach_port_t machPort, bool systemwide) {
  @autoreleasepool {
    ...
    if (messageType == XPC_TYPE_DICTIONARY) {
      ...
      JBLogDebug("received %s message %d with dictionary: %s", systemwide ? "systemwide" : "", msgId,
                 description);
      free(description);

      BOOL isAllowedSystemWide = msgId == JBD_MSG_PROCESS_BINARY || msgId == JBD_MSG_DEBUG_ME ||
                                 msgId == JBD_MSG_SETUID_FIX || msgId == JBD_MSG_FORK_FIX;
...
int main(int argc, char *argv[]) {
  @autoreleasepool {
    ...
    if (kr != KERN_SUCCESS) {
      JBLogError("Failed com.opa334.jailbreakd.systemwide bootstrap check in: %d (%s)", kr,
    ...
    dispatch_source_t source = dispatch_source_create(DISPATCH_SOURCE_TYPE_MACH_RECV, (uintptr_t)machPort, 0,
    ...
    dispatch_source_t sourceSystemWide = dispatch_source_create(
        DISPATCH_SOURCE_TYPE_MACH_RECV, (uintptr_t)machPortSystemWide, 0, dispatch_get_main_queue());
...
static __attribute__((constructor(101), visibility("hidden"))) void BypassJetsam(void) {
  pid_t me = getpid();
  int rc;
  memorystatus_priority_properties_t props = {JETSAM_PRIORITY_CRITICAL, 0};
